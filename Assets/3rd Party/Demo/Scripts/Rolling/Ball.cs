...
namespace InputSamples.Demo.Rolling
{
    ...
    public class Ball : MonoBehaviour
    {
        ...
        [SerializeField]
        private float gravity = -20.0f; // The acceleration due to gravity

        private float jumpVelocity; // The initial velocity of the ball when it jumps
        ...
        protected virtual void FixedUpdate()
        {
            ...
            // Call ContinousJump method to jump the ball when it's grounded
            ContinousJump();
        ...
        // Continuously jumps the ball when it's grounded
        private void ContinousJump()
        {
            if (grounded)
            {
                // Calculate the jump velocity based on the desired jump height and time to reach the peak of the jump
                jumpVelocity = Mathf.Abs(gravity) ;
                cachedRigidbody.velocity = new Vector3(cachedRigidbody.velocity.x, jumpVelocity, cachedRigidbody.velocity.z);
            }
        }
        ...
        {
            cachedRigidbody.AddForce(new Vector3(0.0f, gravity * cachedRigidbody.mass, 0.0f), ForceMode.Force);
        }
    }
}
